...
class XLSXWriter
{
	...
	public function MywriteSheetRow($sheet_name, array $row,$date_time=FALSE)
	{
		if (empty($sheet_name) || empty($row))
			return;
		...
		else
		{
			foreach ($row as $k => $v) {
				if($k==0){
					$this->writeCell($sheet->file_writer, $sheet->row_count, $column_count, $v, "date");
				}
				else if($k==1 || $k==2)
				{
					$this->writeCell($sheet->file_writer, $sheet->row_count, $column_count, $v, "datetime");
				}
				else{
					$this->writeCell($sheet->file_writer, $sheet->row_count, $column_count, $v, $sheet->columns[$column_count]);
				}
