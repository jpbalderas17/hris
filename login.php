...
<html lang="en">
  ...
  function SignUpForm(){
	  ...
	  var password = $("#su_password").val();
		...
		if(!passwordValidation(password)){
			$("#SignUpFormAlert").html("Password may contain only letters, numbers and should be longer than 6 characters.<br/>");
			
		}
		if($("#employee").val()==""){
			$("#SignUpFormAlert").html($("#SignUpFormAlert").html()+" Select Employee.<br/>");
		}
		if($("#su_username").val()==""){
			$("#SignUpFormAlert").html($("#SignUpFormAlert").html()+" Enter Username.<br/>");
		}
		if($("#su_password").val()==""){
			$("#SignUpFormAlert").html($("#SignUpFormAlert").html()+" Enter Password.<br/>");
		}
		if(!$("#SignUpFormAlert").html()==""){
			$("#SignUpFormAlert").show();	
			return
		}
		var employee=$("#employee").val();
		var username=$("#su_username").val();
    $.post("service.php", {'a':'na','password':password,'employee':employee,'username':username}, function(data) {
		  if(data.status == "SUCCESS"){
		  		window.location=top.location=top.location.href
			}else{
				$("#SignUpFormAlert").show();	
				$("#SignUpFormAlert").html(data.message);
			}
	},"json");
  }
  </script>
				...
				<div class="login-form">
					...
					<form id="loginForm" action="login.php" method="POST">
						<fieldset align="center">
							...
							<button class="btn" style="margin-top: 3px;" type="button" onclick="showSignUp();return false;">Sign Up&nbsp;&nbsp;</button>
						</fieldset>
                        <!--<div class="clearfix">
					...
					<form id="SignUpForm" style="display:none;" action="">
						<fieldset align="center">
							<div class="clearfix">
								<div class="input-prepend">
								  	<span class="add-on">Employee:</span>
								  	<select name='employee' id='employee' style="width:300px;" placeholder='Select an Employee'>
								  		<?php
								  			$con=new PDO('mysql:host='.APP_HOST.';dbname='.APP_DB,APP_USERNAME,APP_PASSWORD);
								  			$employees=$con->query("SELECT id,CONCAT(first_name,' ',last_name) as name FROM `employees` WHERE id NOT IN(SELECT employee FROM users)");
								...
								<div class="input-prepend">
								  	...
								  	<input class="span2" type="text" id="su_username" name="username" placeholder="username" style="width:300px;">
								</div>
								<div class="input-prepend">
								  	<span class="add-on">Password:</span>
								  	<input class="span2" type="password" id="su_password" name="password" placeholder="password" style="width:300px;">
								</div>

							</div>
							<div id="SignUpFormAlert" class="clearfix alert alert-info" style="font-size:11px;width:65% 	;margin-bottom:5px;margin-left:auto;margin-right:auto;display:none;"></div>
							<button class="btn" style="margin-top: 3px;margin-left:168px;" type="button" onclick="SignUpForm();return false;">Create Account&nbsp;&nbsp;<span class="icon-arrow-right"></span></button>
						</fieldset>
					</form>
					<?php }else{?>
