...
if($_REQUEST['a'] != "rsp" && $_REQUEST['a'] != "rpc"){
	if(empty($user)){
		 if($_REQUEST['a']=='na'){
				...
				if(empty($_REQUEST['username'])){
					$ret['status']="ERROR";
					$ret['message'].="Enter Password.<br/>";
				}
				if(empty($ret['message'])){
					$con=new PDO('mysql:host='.APP_HOST.';dbname='.APP_DB,APP_USERNAME,APP_PASSWORD);
					$input['username']=$_REQUEST['username'];
					$input['password']=md5($_REQUEST['password']);
					$input['employee']=$_REQUEST['employee'];

					$stmt=$con->prepare("SELECT COUNT(id) FROM users WHERE username=?");
					$stmt->execute(array($input['username']));
					$count=$stmt->fetchColumn();

					if(!empty($count)){
						$ret['status']='ERROR';
							$ret['message'].="Username Already Exists.<br/>";
					}
					else{
						$stmt=$con->prepare("INSERT INTO users(username,password,employee,user_level) VALUES(:username,:password,:employee,'Employee')");
					
						if($stmt->execute($input)){
							$ret['status']='SUCCES';
							$ret['message'].="Account Created.<br/>";
						}
						else{
							$ret['status']="ERROR";
							$ret['message'].="Account Creation Failed.<br/>";
						}	
